...
class PerformanceAnalyzer:
    """
    Comprehensive Performance Analysis for Hockey Backtesting Results - ENHANCED
    ...
    def __init__(self, results_dir: Optional[str] = None):
        ...
        Args:
            results_dir: Directory containing backtesting results (None = use PATHS.experiments)
        """
        # === MIGRACE: Použij PATHS místo hardcoded paths ===
        if results_dir is None:
            self.results_dir = PATHS.experiments
        else:
            self.results_dir = PATHS.root / results_dir
        ...
        logger.info("📊 Enhanced PerformanceAnalyzer initialized")
        logger.info(f"   Results directory: {self.results_dir}")
        logger.info(f"   Enhanced infrastructure: ✅ Paths, Logging, File Handlers")
    ...
    def load_backtest_results(self, 
        ...
        """
        Load backtesting results from JSON file - ENHANCED
        ...
        ENHANCED FEATURES:
        - Safe JSON loading with encoding detection
        - Performance timing
        - PATHS-based file discovery
        """
        self.perf_logger.start_timer('load_backtest_results')
        
        if result_file is None:
            # === MIGRACE: PATHS-based file discovery ===
            patterns = [
                '*backtest*.json',
                '*optimization*.json', 
                '*results*.json'
            ...
            for search_pattern in patterns:
                files = list(self.results_dir.glob(search_pattern))
            ...
            if not all_files:
                error_msg = f"No results files found in: {self.results_dir}"
            ...
            for i, file in enumerate(all_files[:5]):  # Show top 5
                ...
                logger.info(f"   {age_indicator} {file.name} ({file_time.strftime('%Y-%m-%d %H:%M')})")
            
            if len(all_files) > 5:
                logger.info(f"   ... and {len(all_files) - 5} more files")
            
            result_file = all_files[0]  # Latest file
            logger.info(f"🎯 Selected: {result_file.name}")
        else:
            # Convert to Path if string provided
            result_file = self.results_dir / result_file if isinstance(result_file, str) else result_file
        
        # === MIGRACE: Safe JSON loading s encoding detection ===
        try:
            results = read_json(result_file)
            logger.info(f"✅ Results loaded from {result_file.name}")
        ...
        self.perf_logger.end_timer('load_backtest_results')
        return results
    ...
    def analyze_quarterly_performance(self, 
        ...
        """
        Analyze performance broken down by quarters for consistency validation - ENHANCED
        ...
        """
        self.perf_logger.start_timer('quarterly_analysis')
        ...
        try:
            ...
            self.perf_logger.end_timer('quarterly_analysis')
            return analysis
    ...
    def analyze_strategy_comparison(self, 
        ...
        """
        Compare multiple strategies from optimization results - ENHANCED
        ...
        """
        self.perf_logger.start_timer('strategy_comparison')
        logger.info(f"📄 Analyzing strategy comparison (top {top_n} strategies)...")
        
        try:
            # === MIGRACE: Safe file loading ===
            if isinstance(optimization_results, str):
                # Single file path
                file_path = self.results_dir / optimization_results if not optimization_results.startswith('/') else optimization_results
                opt_results = read_json(file_path)
                ...
                for file in optimization_results:
                    file_path = self.results_dir / file if not file.startswith('/') else file
                    data = read_json(file_path)
                    opt_results['optimization_results'].extend(data.get('optimization_results', []))
                logger.info(f"Combined results from {len(optimization_results)} files")
            else:
                opt_results = optimization_results
            
            if not opt_results.get('optimization_results'):
                error_msg = 'No optimization results found'
                logger.error(error_msg)
                return {'error': error_msg}
            ...
            if not strategy_data:
                error_msg = 'No valid strategies found'
                logger.error(error_msg)
                return {'error': error_msg}
            ...
            self.perf_logger.end_timer('strategy_comparison')
            return analysis
    ...
    def analyze_model_performance(self, results: Dict[str, Any]) -> Dict[str, Any]:
        """
        Analyze model prediction performance separate from betting performance - ENHANCED
        ...
        - Better error handling
        """
        self.perf_logger.start_timer('model_analysis')
        ...
        try:
            ...
            if not all_predictions:
                error_msg = 'No prediction data found in results'
            ...
            self.perf_logger.end_timer('model_analysis')
    ...
    def generate_comprehensive_report(self, 
        ...
        """
        Generate comprehensive performance analysis report - ENHANCED
        ...
        ENHANCED FEATURES:
        - Performance timing for full report
        - Better error handling with component isolation
        - Enhanced report structure
        """
        self.perf_logger.start_timer('comprehensive_report')
        ...
        if results_format == 'backtest_single':
            ...
            # Original analysis methods with enhanced error handling
        ...
        self.perf_logger.end_timer('comprehensive_report')
        return report
    ...
    def save_analysis_results(self, 
        ...
        """
        Save analysis results to files - ENHANCED
        ...
        ENHANCED FEATURES:
        - PATHS-based output directory
        - Safe JSON writing
        - Performance timing
        """
        self.perf_logger.start_timer('save_results')
        
        # === MIGRACE: Use PATHS místo hardcoded directory ===
        if output_dir is None:
            save_dir = self.results_dir
        else:
            save_dir = PATHS.root / output_dir
        
        save_dir.mkdir(parents=True, exist_ok=True)
        
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        
        # Main analysis (JSON) - use safe JSON writing
        main_file = save_dir / f'{filename_prefix}_{timestamp}.json'
        
        try:
            write_json(analysis, main_file)
    ...
    def analyze_best_strategies(self, results: Dict[str, Any]) -> Dict[str, Any]:
        """
        Analyze best strategies from optimization results - ENHANCED
        ...
        - Better error handling
        """
        self.perf_logger.start_timer('best_strategies_analysis')
        logger.info("🏆 Analyzing best strategies from optimization...")
        
        if not results.get('best_strategies'):
            error_msg = 'No best strategies found in optimization results'
            logger.error(error_msg)
            return {'error': error_msg}
        ...
        try:
            ...
            self.perf_logger.end_timer('best_strategies_analysis')
            return analysis
            
        except Exception as e:
            logger.error(f"Best strategies analysis failed: {e}")
            self.perf_logger.end_timer('best_strategies_analysis')
            raise
    
    def analyze_parameter_sensitivity(self, results: Dict[str, Any]) -> Dict[str, Any]:
        """
        Analyze parameter sensitivity from optimization results - ENHANCED
        ...
        - Better error handling
        """
        self.perf_logger.start_timer('parameter_sensitivity')
        ...
        try:
            ...
            self.perf_logger.end_timer('parameter_sensitivity')
            return analysis
...
def run_comprehensive_analysis(results_file: Optional[str] = None, 
    ...
    """
    Run comprehensive performance analysis - ENHANCED
    ...
    - Performance timing for entire operation
    - Better error handling
    """
    # === MIGRACE: Setup enhanced logging if not already setup ===
    try:
        setup_logging(
            log_level='INFO',
            log_to_file=True,
    ...
    logger.info("🚀 Starting enhanced comprehensive performance analysis...")
    
    analyzer = PerformanceAnalyzer()
    perf_timer = PerformanceLogger(logger)
    
    perf_timer.start_timer('full_analysis')
    ...
    try:
        ...
        if show_available_files and results_file is None:
            ...
            for pattern in patterns:
                files = list(analyzer.results_dir.glob(pattern))
                all_files.extend(files)
            ...
            if all_files:
                ...
                for i, file in enumerate(all_files):
                    file_time = datetime.fromtimestamp(file.stat().st_mtime)
                    print(f"   {i+1}. {file.name} ({file_time.strftime('%Y-%m-%d %H:%M')})")
                ...
                try:
                    ...
                    if choice:
                        ...
                        if 0 <= idx < len(all_files):
                            ...
                            print(f"Selected: {results_file.name}")
        ...
        perf_timer.end_timer('full_analysis')
...
if __name__ == "__main__":
    ...
    # Ensure logs directory exists (PATHS will handle this)
